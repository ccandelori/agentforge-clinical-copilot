...
/**
 * Internal endpoint: GET /agentforge/internal/notes_search
 *  ?pid=N&q=TEXT&limit=N&since_days=D
 *
 * Called by the Python sidecar's search_notes tool. The sidecar forwards
 * the user-bound JWT it received from the browser; we validate it with the
 * same shared secret. Production deployments should also restrict this URL
 * path to the sidecar's IP via reverse-proxy ACL — the JWT check is
...
$controller = new InternalNotesSearchController(
