...
/**
 * Internal endpoint: POST /agentforge/internal/log_breakglass
 *
 * Called by the Python sidecar each time a breakglass-flagged user opens
 * a patient chart through the agent. The sidecar forwards the user-bound
 * JWT it received from the browser; we validate it with the same shared
 * secret, then write an audit row through OpenEMR's existing
 * EventAuditLogger so the breakglass reason lands in the encrypted
 * `comments` column.
 *
 * Production deployments should also restrict this URL path to the
 * sidecar's IP via reverse-proxy ACL — the JWT check is defense-in-depth
...
$controller = new InternalBreakglassController(
    new AgentJwtValidator($secret, ServiceContainer::getClock()),
    new BreakglassAuditWriter($connection, EventAuditLogger::getInstance()),
);

$response = $controller->handle(Request::createFromGlobals());
$response->send();
