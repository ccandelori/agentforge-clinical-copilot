...
add defense-in-depth network filtering.

## `apache-agentforge.conf`

Apache 2.4 include. **Defense-in-depth on internal endpoints** — restricts
...
traffic gets a 403 at the network edge before PHP runs. The JWT check
inside each endpoint is still the primary security gate; this is a
second layer.
