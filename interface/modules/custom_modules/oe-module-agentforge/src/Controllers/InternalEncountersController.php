...
/**
 * InternalEncountersController — sidecar-facing read-only endpoint that
 * serves the patient's recent encounters from form_encounter for the
 * agent's get_recent_encounters tool.
 *
 ...
 *
 * The optional `since_days` query parameter narrows the lookback window
 * (default 365, clamped server-side to [1, 730]). Encounters span longer
 * windows than labs/notes, so a year+ default is more useful than the
 * 90-day default the notes endpoint uses.
 *
 * @package   OpenEMR
...
class InternalEncountersController
{
    public const DEFAULT_SINCE_DAYS = 365;
    public const MIN_SINCE_DAYS = 1;
    public const MAX_SINCE_DAYS = 730;

    public function __construct(
        private readonly AgentJwtValidator $validator,
        private readonly EncountersRepository $repository,
    ) {
    }
    ...
    public function show(Request $request): Response
    {
        ...
        $encounters = $this->repository->findRecentByPid($pid, $since);
        return new JsonResponse(['encounters' => $encounters]);
    }
