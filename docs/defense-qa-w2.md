...
- [`docs/eval-report-2026-05-03.md`](./eval-report-2026-05-03.md) — most recent eval report on file at primer authoring time (W2 50-case report per Task 31 is still pending; see Q5)
...
**Answer.** A separate test, [`sidecar/tests/eval/test_gate_blocks_regression.py`](../sidecar/tests/eval/test_gate_blocks_regression.py) (Task 19 in the W2 task plan), monkey-patches the `intake-extractor` worker to return one fabricated lab value, runs the full 50-case suite against the patched agent, and asserts that the rubric category `factually_consistent` drops by more than 5% — i.e., that the build *would* fail. It runs as a separate gate-validation job (it always fails by design) and is also runnable by graders directly on demand. We considered the alternative — seeding intentionally-broken cases into the 50-case golden set — and rejected it because mixing pass and fail expectations into one dataset makes the rubric ambiguous and invites future engineers to "fix" the broken cases. The honest caveat: the W2 baseline at `sidecar/baselines/week2.json` ships as a structural stub (per the Task 25 spec — "stub containing the expected structure: per-category pass rates, total cases, timestamp") and is *replaced* by Task 31's report run against the deployed agent. At primer authoring time the most recent eval report on file is [`docs/eval-report-2026-05-03.md`](./eval-report-2026-05-03.md) — a 12-case W1 run, all passing — and the W2 50-case `eval-report-2026-05-04.md` per Task 31's spec is still pending. The gate's *self-test* therefore proves the gate bites; the *baseline* it bites against is only authoritative once Task 31 ships, and that is the visible loose end at the time of the defense.
...
- **Task 31's W2 50-case eval report (`docs/eval-report-2026-05-04.md`) has not yet been authored.** The most recent eval report on file is the 12-case W1 run at `docs/eval-report-2026-05-03.md`. The W2 baseline at `sidecar/baselines/week2.json` is structurally a stub per Task 25 until Task 31 replaces it with a run against the deployed agent. The gate self-test (Q5) proves the gate *bites*; the absolute baseline it bites against is only authoritative once Task 31 ships.
- **Task 19 (`test_gate_blocks_regression.py`) and Task 21 (Dockerfile pre-baked weights) are both pending at primer authoring time.** Q5 and Q6 above describe the design intent; the as-shipped status check should be `task-master show 19` / `task-master show 21` immediately before the defense.
...
*Last updated 2026-05-08. Author: Cameron Candelori (with `Assisted-by: Claude Code`).*
