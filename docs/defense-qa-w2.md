...
- [`docs/eval-report-2026-05-08.md`](./eval-report-2026-05-08.md) — the W2 50-case report (Task 31 shipped); the report itself notes the pinned baseline is a stub pending the production Supervisor-adapter regen, so the gate's *self-test* is what proves the gate bites today (see Q5)
...
- **Stub baseline.** [`sidecar/tests/eval/baselines/week2.json`](../sidecar/tests/eval/baselines/week2.json) is structurally pinned at 1.0 across all five W2 categories (`_meta.status: "stub"`). Real per-category pass rates require running `python -m agentforge.eval.regenerate_baseline` against the live agent on the droplet — that's the W2 deferred-work item. The production Supervisor adapter ([`sidecar/src/agentforge/eval/supervisor_adapter.py`](../sidecar/src/agentforge/eval/supervisor_adapter.py)) ships as the bridge to make that regen possible; the human-edit `_build_real_supervisor_and_harness` in the regen CLI is the last remaining piece before a measured baseline lands.
