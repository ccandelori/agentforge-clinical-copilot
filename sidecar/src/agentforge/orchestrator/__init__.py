...
get_active_medications, and get_vitals_trend. We rely on Claude's
stop_reason to know when to stop tool-calling; we cap the iteration count
so a misbehaving model can't burn the deadline. When the verifier ships
(Task 28) and the rest of the tools land (Tasks 17-25), the loop here
...
results, allergies, encounter history, imaging, etc.), name the gap \
